...
export async function GET(request: NextRequest) {
  ...
  const { rows: claimed } = await pool.query<{ user_id: string }>(
    `UPDATE alarm_settings a
      ...
      RETURNING a.user_id`,
    [dateStr, nowMinutes - WINDOW_MINUTES, nowMinutes, todayMidnight.toISOString()],
  );
  ...
  return NextResponse.json({ ok: true, sent, claimed: userIds.length, removed: invalidTokens.length });
}
